...
def solve_sparse_dp_numba_core(
    ...
    future_data = np.zeros((MAX_TRANSITIONS, 4), dtype=np.float64)  # t, state, cost, pen
    n_future = 0
    
    # Parent tracking for backtracking
    # parent[state_hash] = (prev_t, prev_state, length)
    parent_data = np.zeros((MAX_TRANSITIONS, 5), dtype=np.int64)  # t, state, prev_t, prev_state, L
    n_parent = 0
    ...
    for t in range(T + 1):
        ...
        if final_idx >= 0 and idx_to_state[final_idx] == final_state:
            ...
            if c < best_final_cost or (abs(c - best_final_cost) < _EPS and p < best_final_pen):
                best_final_cost = c
                best_final_pen = p
                best_final_time = t
        ...
        for idx in range(n_states_curr):
            ...
            if existing_idx < 0 or next_idx_to_state[existing_idx] != state:
                ...
                if c0 < next_state_cost[existing_idx] or \
                   (abs(c0 - next_state_cost[existing_idx]) < _EPS and p0 < next_state_pen[existing_idx]):
            ...
            if f_t == t + 1:
                ...
                if existing_idx < 0 or idx_to_state[existing_idx] != f_state:
                    ...
                    if f_cost < state_cost[existing_idx] or \
                       (abs(f_cost - state_cost[existing_idx]) < _EPS and f_pen < state_pen[existing_idx]):
...
def solve_sparse_dp_numba(
    ...
    # Compute radices and multipliers for state encoding
    ...
    # Convert to proper types for Numba
    lengths = np.asarray(lengths, dtype=np.int32)
    totals = np.asarray(totals, dtype=np.int32)
    prefix = np.asarray(prefix, dtype=np.float64)
    radices = np.asarray(radices, dtype=np.int32)
    mult = np.asarray(mult, dtype=np.int64)
    
    start_time = time.perf_counter()
    time_limit_ticks = int(time_limit * 1e9) if time_limit > 0 else -1
    
    # Run the core DP
    best_cost, best_t, parent_data, timed_out = solve_sparse_dp_numba_core(
        lengths, totals, prefix, T, radices, mult, final_state,
        time_limit_ticks, int(start_time * 1e9)
    )
    
    # Check timeout at Python level
    ...
    # Backtrack to recover schedule (simplified - just return cost for now)
    # Full backtracking would require more complex parent tracking
    return DPResultNumba(
        feasible=True,
        cost=float(best_cost),
        finish_time=int(best_t),
        segments=(),  # TODO: implement backtracking
        timed_out=timed_out,
    )
