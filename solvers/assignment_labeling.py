...
def score_assignment_l2(
    *,
    ...
    for mi in range(m):
        ...
        load = int(sum(int(p[j]) for j in job_ids))
        if load > K_i:
            per_machine_best.append(float("inf"))
            feasible = False
            continue
        ...
        seqs = [
            ("cheap_first", None),
            ("medium_first", None),
            ("cheap_first", int(seed1)),
            ("cheap_first", int(seed2)),
        ]

        best = float("inf")
        for mode, seed in seqs:
            seq = make_pack_sequence(
                job_ids=job_ids,
                processing_times=p,
                ct=ct,
            ...
            if np.isfinite(float(energy)) and float(energy) < best:
                best = float(energy)
        ...
        per_machine_best.append(float(best))
        total += float(best)

    return (float(total) if feasible else float("inf")), tuple(per_machine_best), bool(feasible)


def score_assignment_l1_l2(
    *,
    instance: RawInstance,
    assignment: Sequence[int],
    K: int,
) -> AssignmentScores:
    l1, per1, feas1 = score_assignment_l1(instance=instance, assignment=assignment, K=K)
    l2, per2, feas2 = score_assignment_l2(instance=instance, assignment=assignment, K=K)
    return AssignmentScores(
        l1_total_energy=float(l1),
        l2_total_energy=float(l2),
        feasible_l1=bool(feas1),
        feasible_l2=bool(feas2),
        per_machine_l1=tuple(float(x) for x in per1),
        per_machine_l2=tuple(float(x) for x in per2),
    )
