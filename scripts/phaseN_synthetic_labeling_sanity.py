...
def _epsilon_from_instance(row: ManifestRow, epsilon_slack: int) -> Tuple[int, int, int]:
    p = _read_int_values(row.data_p_path)
    if not p:
        return 0, 0, 0
    lb = max(int(math.ceil(float(sum(p)) / float(row.m))), max(p))
    epsilon = min(row.k, lb + epsilon_slack)
    return epsilon, lb, int(sum(p))
...
def _read_labeled_moves(path: Path) -> Tuple[List[Dict[str, str]], int]:
    if not path.exists():
        return [], 0
    with path.open("r", encoding="utf-8", newline="") as f:
        reader = csv.DictReader(f)
    ...
    positives = 0
    for row in rows:
        if str(row.get("label_improving", "0")).strip() == "1":
            positives += 1
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Phase N synthetic-only exact-label extraction sanity pass")
    parser.add_argument(
        "--train-manifest",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="temp/phaseN_synthetic_labeling_sanity",
    )
    parser.add_argument(
        "--solver-bin",
        type=str,
    ...
    parser.add_argument("--epsilon-slack", type=int, default=20)
    parser.add_argument("--per-machine-dp-limit-sec", type=float, default=1.0)
    parser.add_argument("--ls-time-cap-sec", type=float, default=1.0)
    parser.add_argument("--ls-max-rounds", type=int, default=1)
    parser.add_argument("--ls-max-moves-per-round", type=int, default=800)
    ...
    source_exact_path = repo_root / "temp/phaseL1_dataset_logging/moves_exact_labeled_61_347.csv"

    for row in selected:
        epsilon, epsilon_lb, total_p = _epsilon_from_instance(row, args.epsilon_slack)
        ...
        exact_rows, positive_rows = _read_labeled_moves(source_exact_path)
    ...
    split_rows: Dict[str, List[Dict[str, object]]] = {"train": [], "val": []}
    ...
    subset_agg_rows = split_rows["train"] + split_rows["val"]
    _write_csv(
    ...
    write_moves(output_dir / "synthetic_moves_exact_labeled_train_sanity.csv", train_moves)
    write_moves(output_dir / "synthetic_moves_exact_labeled_val_sanity.csv", val_moves)
    write_moves(output_dir / "synthetic_moves_exact_labeled_sanity_merged.csv", merged_moves)

    merged_columns = list(merged_moves[0].keys()) if merged_moves else []
    schema_payload = {
        "dataset": "synthetic_moves_exact_labeled_sanity_merged",
    ...
    (output_dir / "feature_schema_sanity.json").write_text(json.dumps(schema_payload, indent=2), encoding="utf-8")

    total_exact = len(merged_moves)
    total_pos = sum(1 for r in merged_moves if str(r.get("label_improving", "0")) == "1")
    ...
    summary_payload = {
        "phase": "phaseN_synthetic_labeling_sanity",
        "manifests_used": {
        ...
        "sample_config": {
            ...
            "epsilon_policy": "epsilon=min(K, ceil(sum(p)/M)+epsilon_slack)",
        ...
        "metrics": {
            "instances_labeled_train": int(split_rows["train"][0]["instances_with_labels"] if split_rows["train"] else 0),
            "instances_labeled_val": int(split_rows["val"][0]["instances_with_labels"] if split_rows["val"] else 0),
        ...
        "readiness_signal": {
            ...
            "ready_to_scale": bool(total_exact > 0 and total_pos > 0 and all_runs_ok and manifest_gating_ok),
        },
    }
