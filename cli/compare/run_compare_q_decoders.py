...
def run_dp_on_sequence(env, batch, experiment_results):
    ...
    for i, res in enumerate(experiment_results):
        ...
        energy, _ = _dp_schedule_fixed_order(
            processing_times=proc, ct=ct, e_single=e_single, T_limit=T_limit
        )
...
def run_compare_decoders(args):
    ...
    for i in range(args.num_instances):
        ...
        raw = generate_raw_instance(current_config, py_rng, instance_id=i)
        raw_instances.append(raw)

    # 3. Run Decoders
    results = []

    decoding_modes = [("Standard (ASAP)", False), ("DP-Guided", True)]
    ...
    for i, raw_instance in enumerate(raw_instances):
        ...
        mh_assignment = simulate_metaheuristic_assignment(
            raw_instance.n, raw_instance.m, py_rng
        )
        ...
        episode = make_single_machine_episode(
            ...
            job_idxs,
            py_rng,
            deadline_slack_ratio_min=0.2,
            deadline_slack_ratio_max=0.5,
        ...
        ratio = (episode.T_limit - episode.T_min) / (
            episode.T_max - episode.T_min + 1e-6
        )  # Approximate
        ...
        for mode_name, use_dp_guided in decoding_modes:
            ...
            if use_dp_guided:
                ...
                base_model_for_sgbs = (
                    model.q_model if hasattr(model, "q_model") else model
                )
    ...
    summary = (
        df.groupby(["decoder"])["energy"].mean().reset_index().sort_values("energy")
    )
    print("\n" + "=" * 50)
