"""Run evaluation for PaST-SM with SGBS and baselines.

Methods:
- greedy model decode
- SGBS(beta, gamma) (paper-faithful Algorithm 1)
...
This script is intentionally standalone and does not modify existing training/eval code.

Example:
    python -m PaST.cli.eval.run_eval_sgbs_and_baselines \
    --run_dir runs_p100/ppo_short_base/seed_0 \
    --which best \
    --eval_seed 1337 \
    --num_instances 64 \
    --beta 4 --gamma 4
...
def _load_checkpoint(path: Path, device: torch.device) -> Dict[str, Any]:
    ...
    # PyTorch 2.6+ defaults weights_only=True, which can reject older checkpoint
    # formats that include non-tensor pickled objects.
    try:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate checkpoint with SGBS + baselines",
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Variant id like ppo_short_base (required with --checkpoint).",
    )
    p.add_argument(
        "--which",
        type=str,
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="CSV output path (defaults to <run_dir>/eval_sgbs_<which>_seed<seed>_b<beta>_g<gamma>.csv)",
    )
    p.add_argument(
        "--out_json",
        type=str,
...
def main() -> None:
    ...
    out_csv = (
        ...
        / f"eval_sgbs_{ckpt_tag}_seed{args.eval_seed}_b{args.beta}_g{args.gamma}.csv"
    )
