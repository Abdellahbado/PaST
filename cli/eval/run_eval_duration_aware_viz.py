...
# Add project root to sys.path to allow running from script directory or root without setting PYTHONPATH
...
def _action_trace_to_bars_family(
    ...
    """Convert action trace to bars for price-family variant with BEST-START decoding.

    In price-family mode, action = job_id * num_families + family_id.
    This version implements the 'best start' logic: find the valid start time
    within the chosen family that MINIMIZES energy cost.
    """
    ...
    # Pre-calculate cumulative sum of energy prices for O(1) interval cost query
    # cumsum[x] = sum(ct[:x])
    ct_cumsum = np.zeros(len(ct) + 1, dtype=np.float64)
    ct_cumsum[1:] = np.cumsum(ct)

    wait_penalty = float(getattr(env_config, "best_family_start_wait_penalty", 0.0))
    max_wait_slots = getattr(env_config, "best_family_start_max_wait_slots", None)
    ...
    # Calculate remaining work (sum of p for all jobs)
    # This is needed to check the 'completion' constraint: t + remaining_work <= T_limit
    # We will decrement this as we schedule jobs.
    remaining_work = np.sum(p_subset)
    ...
    for a in actions:
        ...
        # Find BEST slot in the target family.
        # Match env BEST-START: minimize (energy_window_cost + wait_penalty * wait).
        # Note: energy reported should be based on the true window energy only (no wait penalty).
        best_start = None
        best_obj = float("inf")
        best_energy_window_cost = float("inf")

        # Range of valid start times:
        # 1. Must be >= current time t
        # 2. Must allow job to finish: start + p <= T_limit
        # 3. Must allow REMAINING work to finish: start + remaining_work <= T_limit
        #    (start + p + (remaining_work - p) <= T_limit) -> start + remaining_work <= T_limit

        # Wait, the environment logic `completion_ok` is:
        # slot + remaining_work <= T_limit.
        # Here `remaining_work` includes current job p.

        max_valid_start = T_limit - remaining_work
        # It also has to satisfy local deadline: start + p <= T_limit
        max_valid_start = min(max_valid_start, T_limit - p)

        # If we are already late, just try to start as soon as possible (t)
        # But strictly speaking we should search between [t, max_valid_start]

        search_end = max(t, max_valid_start) + 1  # exclusive
        # Cap search at T_limit just in case
        search_end = min(search_end, T_limit - p + 1)

        if max_wait_slots is not None and int(max_wait_slots) >= 0:
            search_end = min(search_end, t + int(max_wait_slots) + 1)

        # Iterate through valid slots
        # Note: In strict env usage, we iterate up to T_limit, but filter by valid_start & completion_ok
        ...
        for u in range(t, search_end):
            if slot_families[u] == family_id:
                ...
                if obj < best_obj:
                    best_obj = obj
                    best_start = u
                    best_energy_window_cost = energy_cost

        # If no valid slot found in family (e.g. search range empty or no family match),
        # what does the environment do?
        # Env returns T_limit (invalid). But for visualization we must show *something*.
        # Fallback: just start at t (earliest possible), effectively ignoring family constraint.
        if best_start is None:
            best_start = t
            best_energy_window_cost = get_interval_cost(t, t + p)
        ...
        if end > T_limit:
            ...
            start = max(0, end - p)
            best_energy_window_cost = get_interval_cost(start, end)

        energy = float(e_single) * float(best_energy_window_cost)
        total_energy += energy
        ...
        # Update remaining work for next job
        remaining_work -= p
...
def plot_epsilon_curves(
    ...
    for m in methods:
        ...
        ax1.plot(
            slacks, energies, label=m, **styles.get(m, {}), color="tab:blue", alpha=0.7
        )
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(
        description="Evaluate ppo_family_q4_beststart with SGBS + baselines and visualization",
    ...
    p.add_argument(
        "--out_dir", type=str, default=None, help="Directory for visualization outputs"
    )
    ...
    p.add_argument(
        "--epsilon_constraint",
        action="store_true",
        help="Run epsilon-constraint loop analysis",
    )
    p.add_argument(
        "--epsilon_steps", type=int, default=10, help="Number of steps for epsilon loop"
    )
...
def main() -> None:
    ...
    # CRITICAL: Force best_family_start=True for this evaluation script
    # to ensure the environment decoding matches the visualization logic
    # and the user's intent, even if the variant config default is different.
    ...
    if args.epsilon_constraint:
        run_epsilon_constraint_analysis(
            args, variant_config, data_cfg, model, device, scale_str
        )
        return
    ...
    print("=" * 70)
    print(
        f"{variant_id_str} | {args.which}.pt | seed={args.eval_seed} | N={args.num_instances} | scale={scale_str}"
    )
    ...
    if args.num_viz > 0:
        ...
        for viz_idx in range(min(int(args.num_viz), int(args.num_instances))):
            ...
            out_png = (
                out_dir
                / f"schedule_viz_{scale_str}_seed{args.eval_seed}_idx{viz_idx}.png"
            )
            visualize_schedule(
                out_png,
                {
    ...
    out_csv = (
        ...
        / f"eval_family_{scale_str}_{ckpt_tag}_seed{args.eval_seed}_b{args.beta}_g{args.gamma}.csv"
    )
