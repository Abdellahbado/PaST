"""
EAS Evaluation Script for ppo_short_base variant.
...
Example:
    python PaST/run_eval_eas_ppo_short_base.py \\
...
def run_evaluation(
    ...
    print("\n" + "=" * 70)
    print(f"EAS EVALUATION - {args.method.upper()}")
    ...
    out_csv = out_dir / f"eas_results_{scale_str}_seed{args.eval_seed}.csv"
    ...
    # Plot results
    plot_results(df, out_dir, scale_str, args.eval_seed)
...
def plot_results(df: pd.DataFrame, out_dir: Path, scale_str: str, seed: int):
    ...
    ax.set_title(f"EAS Evaluation - {scale_str} instances")
    ax.legend()
    ax.grid(True, alpha=0.3)

    plt.tight_layout()
    out_path = out_dir / f"eas_plot_{scale_str}_seed{seed}.png"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="EAS Evaluation for ppo_short_base")
