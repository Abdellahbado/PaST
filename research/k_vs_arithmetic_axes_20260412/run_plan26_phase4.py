#!/usr/bin/env python3
"""PLAN26 Phase 4: Step-3 beam scoring variants (available ones only)."""
...
def main():
    ...
    # Phase 4: Step-3 beam scoring variants (available ones)
    # Note: residual_aware and late_ambig require new implementation, not available
    for variant in ["standard_step4", "ambig_scoreband_mult2"]:
        if variant == "standard_step4":
            env = make_variant_env("standard_step4")
            tlim = 1200.0
        elif variant == "ambig_scoreband_mult2":
            env = make_variant_env("standard_step4")
            env["PAST_PROFILE_REPAIR_BEAM_KEY_MULTI_POLICY"] = "ambig_scoreband"
            env["PAST_PROFILE_REPAIR_BEAM_KEY_MULTI_MAX"] = "2"
            tlim = 1200.0
        for fam, seed in [("hardA_k10", 0), ("hardA_k10", 2), ("hardB_k10", 0), ("hardB_k10", 2)]:
            plan.append((variant, env, fam, 1000, seed, tlim, 16.0))
    ...
    for label, env, fam, n, seed, tlim, mem in plan:
        ...
        print(f"{label} family={fam} seed={seed} step={r.get('deciding_step')} ub={r.get('ub')} gap={r.get('gap_pct')} rt={r.get('runtime_sec')} rss={r.get('peak_rss_gb')} beam_status={r.get('fwd_profile_beam_status')}")
