#!/usr/bin/env python3
"""PLAN25: local corridor exact DP diagnostic."""
...
OUT_DIR = ROOT / "research" / "k_vs_arithmetic_axes_20260412" / "csv" / "plan25"
RAW_CSV = OUT_DIR / "PLAN25_local_corridor_dp_raw.csv"
...
def run_row(family_id, n_jobs, seed, time_limit, variant_label, env_overrides, max_rss_gb=16.0):
    ...
    t0 = time.monotonic()
    out_file = tempfile.NamedTemporaryFile(prefix="plan25_out_", suffix=".txt", delete=False)
    err_file = tempfile.NamedTemporaryFile(prefix="plan25_err_", suffix=".txt", delete=False)
    ...
    row.update({
        ...
        # PLAN25 local corridor diagnostics
...
def main():
    ...
    # Phase B: hardB_k10 seed=2 (only run if smoke passes; included here for convenience)
    for variant in ["standard_step4", "local_corridor_delta1_300s", "local_corridor_delta2_300s"]:
        if variant == "standard_step4":
            env = make_variant_env("standard_step4")
            tlim = 1200.0
    ...
    for label, env, fam, n, seed, tlim, mem in plan:
        ...
        print(f"{label} family={fam} seed={seed} step={r.get('deciding_step')} ub={r.get('ub')} gap={r.get('gap_pct')} rt={r.get('runtime_sec')} rss={r.get('peak_rss_gb')} local_status={r.get('local_corridor_status')} local_stop={r.get('local_corridor_stop_reason')}")
