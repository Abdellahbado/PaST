...
OUT_DIR = ROOT / "research" / "k_vs_arithmetic_axes_20260412" / "csv" / "plan26"
RAW_CSV = OUT_DIR / "PLAN26_multi_idea_raw.csv"
...
def run_row(family_id, n_jobs, seed, time_limit, variant_label, env_overrides, max_rss_gb=16.0):
    ...
    t0 = time.monotonic()
    out_file = tempfile.NamedTemporaryFile(prefix="plan26_out_", suffix=".txt", delete=False)
    err_file = tempfile.NamedTemporaryFile(prefix="plan26_err_", suffix=".txt", delete=False)
    ...
    row.update({
        ...
        # PLAN26 local corridor diagnostics
        "local_corridor_enabled": _extract(raw, "local_corridor_enabled"),
        "local_corridor_delta": _extract(raw, "local_corridor_delta"),
        "local_corridor_status": _extract(raw, "local_corridor_status"),
        "local_corridor_layers": _extract(raw, "local_corridor_layers"),
        "local_corridor_states_seen": _extract(raw, "local_corridor_states_seen"),
        "local_corridor_states_kept_max": _extract(raw, "local_corridor_states_kept_max"),
        "local_corridor_states_pruned": _extract(raw, "local_corridor_states_pruned"),
        "local_corridor_transitions_considered": _extract(raw, "local_corridor_transitions_considered"),
        "local_corridor_transitions_kept": _extract(raw, "local_corridor_transitions_kept"),
        "local_corridor_time_sec": _extract(raw, "local_corridor_time_sec"),
        "local_corridor_best_ub": _extract(raw, "local_corridor_best_ub"),
        "local_corridor_closed": _extract(raw, "local_corridor_closed"),
        "local_corridor_stop_reason": _extract(raw, "local_corridor_stop_reason"),
        "local_corridor_memory_safe": _extract(raw, "local_corridor_memory_safe"),
        "local_corridor_beam_counts_size": _extract(raw, "local_corridor_beam_counts_size"),
        "local_corridor_merged_blocks": _extract(raw, "local_corridor_merged_blocks"),
        "local_corridor_block_count_mismatch": _extract(raw, "local_corridor_block_count_mismatch"),
        "local_corridor_target_offset_l1": _extract(raw, "local_corridor_target_offset_l1"),
        "local_corridor_target_in_corridor": _extract(raw, "local_corridor_target_in_corridor"),
        "local_corridor_base_candidates_finite": _extract(raw, "local_corridor_base_candidates_finite"),
        "local_corridor_empty_candidate_blocks": _extract(raw, "local_corridor_empty_candidate_blocks"),
        "local_corridor_first_empty_layer": _extract(raw, "local_corridor_first_empty_layer"),
        "local_corridor_base_path_survives": _extract(raw, "local_corridor_base_path_survives"),
        "local_corridor_base_path_cost": _extract(raw, "local_corridor_base_path_cost"),
        "local_corridor_base_path_reject_reason": _extract(raw, "local_corridor_base_path_reject_reason"),
    })
    return row
...
def main():
    ...
    for variant in ["standard_step4", "local_corridor_delta1_300s", "local_corridor_delta2_300s"]:
        if variant == "standard_step4":
            env = make_variant_env("standard_step4")
            tlim = 1200.0
        elif variant == "local_corridor_delta1_300s":
            env = make_variant_env("standard_step4")
            env["PAST_BEAM_CORRIDOR_LOCAL_DP"] = "1"
            env["PAST_BEAM_CORRIDOR_LOCAL_DELTA"] = "1"
            env["PAST_BEAM_CORRIDOR_LOCAL_TIME_LIMIT"] = "300"
            tlim = 1200.0
        elif variant == "local_corridor_delta2_300s":
            env = make_variant_env("standard_step4")
            env["PAST_BEAM_CORRIDOR_LOCAL_DP"] = "1"
            env["PAST_BEAM_CORRIDOR_LOCAL_DELTA"] = "2"
            env["PAST_BEAM_CORRIDOR_LOCAL_TIME_LIMIT"] = "300"
            tlim = 1200.0
    ...
    for label, env, fam, n, seed, tlim, mem in plan:
        ...
        print(f"{label} family={fam} seed={seed} step={r.get('deciding_step')} ub={r.get('ub')} gap={r.get('gap_pct')} rt={r.get('runtime_sec')} rss={r.get('peak_rss_gb')} local_status={r.get('local_corridor_status')} base_survives={r.get('local_corridor_base_path_survives')} mismatch={r.get('local_corridor_block_count_mismatch')} target_l1={r.get('local_corridor_target_offset_l1')} reject={r.get('local_corridor_base_path_reject_reason')}")
