#!/usr/bin/env python3
"""PLAN29: Multi-View Adjacent Block Reconstruction — Phase A."""
...
OUT_DIR = RESEARCH_DIR / "csv" / "plan29"
RAW_CSV = OUT_DIR / "PLAN29_multiview_block_reconstruction_raw.csv"
...
VIEW_VARIANTS = {
    "baseline": {},
    "coarsen2": {"PAST_BLOCK_VIEW_POLICY": "coarsen2"},
    "coarsen3": {"PAST_BLOCK_VIEW_POLICY": "coarsen3"},
    "target_B12": {"PAST_BLOCK_VIEW_POLICY": "target_b", "PAST_BLOCK_VIEW_TARGET_B": "12"},
    "target_B8": {"PAST_BLOCK_VIEW_POLICY": "target_b", "PAST_BLOCK_VIEW_TARGET_B": "8"},
    "price_preserve_B12": {"PAST_BLOCK_VIEW_POLICY": "price_preserve", "PAST_BLOCK_VIEW_TARGET_B": "12"},
    "arith_adaptive": {"PAST_BLOCK_VIEW_POLICY": "arith_adaptive"},
}
...
def run_row(family_id, n_jobs, seed, time_limit, variant_label, env_overrides, max_rss_gb=16.0):
    ...
    t0 = time.monotonic()
    out_file = tempfile.NamedTemporaryFile(prefix="plan29_out_", suffix=".txt", delete=False)
    err_file = tempfile.NamedTemporaryFile(prefix="plan29_err_", suffix=".txt", delete=False)
    ...
    no_incumbent = (fwd_method == "none" or ub_val == "-1" or ub_val == "-1.000000" or fwd_outcome == "failed")
    if no_incumbent:
        row.update({
    ...
    row.update({
        ...
        "block_view_policy": _extract(raw, "block_view_policy"),
        "block_view_original_blocks": _extract(raw, "block_view_original_blocks"),
        "block_view_final_blocks": _extract(raw, "block_view_final_blocks"),
        "block_view_removed_boundaries": _extract(raw, "block_view_removed_boundaries"),
        "block_view_target_b": _extract(raw, "block_view_target_b"),
        "block_view_price_preserve_used": _extract(raw, "block_view_price_preserve_used"),
        "block_view_arith_adaptive_used": _extract(raw, "block_view_arith_adaptive_used"),
        "block_view_selected": _extract(raw, "block_view_selected"),
        "block_view_eval_count": _extract(raw, "block_view_eval_count"),
        "block_view_best_ub": _extract(raw, "block_view_best_ub"),
        "block_view_time_sec": _extract(raw, "block_view_time_sec"),
    })
    return row
...
def main():
    ...
    for fam in families:
        for seed in seeds:
            for variant_label, variant_env in VIEW_VARIANTS.items():
                plan.append((fam, 1000, seed, tlim, variant_label, variant_env))
    ...
    for fam, n, seed, tlim, label, env_ov in plan:
        ...
        print(
            f"var={label:<20} fam={fam:<12} seed={seed} "
            f"step={r.get('deciding_step'):<12} "
            f"ub={r.get('ub'):<18} gap={r.get('gap_pct'):>10} "
            f"rt={r.get('runtime_sec'):>10} rss={r.get('peak_rss_gb'):>6} "
            f"bv_orig={r.get('block_view_original_blocks'):>4} "
            f"bv_final={r.get('block_view_final_blocks'):>4} "
            f"bv_removed={r.get('block_view_removed_boundaries'):>4}"
        )
