...
from sandbox.neurols.operators import Move, Operator, OperatorID, OPERATORS, OPERATOR_BY_ID
...
class CandidateGenerator:
    ...
    def generate_best_move(
        ...
        Args:
            ...
            context: Context for move enumeration (e.g. physics)

        Returns:
            Best MoveEvaluation, or None if no valid moves
        """
        # Enumerate all moves for this operator
        all_moves = operator.enumerate_moves(solution, context=context)
    ...
    def generate_all_operator_moves(
        ...
        Args:
            solution: Current solution
            current_eval: Current solution evaluation
            context: Context for move enumeration
        ...
        for operator in OPERATORS:
            result[operator.id] = self.generate_best_move(
                solution, current_eval, operator, context=context
            )
        return result
